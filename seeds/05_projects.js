export function seed(knex) {
  ...
  return knex('projects')
    .insert(projects)
    .onConflict('id')
    .merge(['name', 'parent_project_id', 'created_by_id', 'created_at'])
    .then(() => {
      // For entity_properties, we'll use a different approach since there might not be a unique constraint
      return knex('entity_properties')
        .del()
        .then(() => {
          return knex('entity_properties').insert(entityProperties);
        });
    });
} 