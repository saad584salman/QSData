...
global.waitForDatabase = async (db) => {
  let retries = 0;
  const maxRetries = 15;
      ...
      if (retries === 1) {
        ...
        try {
          ...
          try {
            ...
            console.log(`⚠️ Migration failed for ${testDbName}: ${migrationError.message}`);
            // Continue anyway, the database might already be migrated
          }
        } catch (createError) {
          console.log(`ℹ️ Database creation failed (might already exist): ${createError.message}`);
        }
      }
...
global.setupTestData = async (db) => {
  try {
    ...
    console.error(`❌ Test data setup failed for ${testDbName}:`, error.message);
    throw error;
  }
};

// Add a function to get test data IDs for use in tests
global.getTestDataIds = async (db) => {
  try {
    ...
    console.log(`⚠️ Error getting test data IDs: ${error.message}`);
