...
import { EntityProperty as EntityPropertyType, QueryOptions, ApiResponse, PaginatedResponse } from '../types/index.js';

export const getEntityProperties = async (req: Request, res: Response): Promise<void> => {
  try {
    const { 
      entity_type, 
      entity_id, 
...
export const getEntityPropertyById = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
...
export const createEntityProperty = async (req: Request, res: Response): Promise<void> => {
  try {
...
export const updateEntityProperty = async (req: Request, res: Response): Promise<void> => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
...
export const deleteEntityProperty = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
